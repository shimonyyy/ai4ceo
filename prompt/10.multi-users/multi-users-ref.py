...
APP_TITLE = "PDF 기반 멀티유저 RAG 챗봇"
MODEL_OPTIONS = ["gpt-5.5", "claude-opus-4-7", "gemini-3-pro-preview"]
EMBEDDING_MODEL = "text-embedding-3-small"
DEFAULT_MATCH_COUNT = 8
DEFAULT_MATCH_THRESHOLD = 0.2
KST = timezone(timedelta(hours=9))
AUTH_SALT = os.getenv("APP_AUTH_SALT", "change-this-salt")
AUTH_DB_PATH = Path(__file__).resolve().parent / "local_auth.db"
...
def load_environment() -> None:
    ...
    # Streamlit Cloud secrets -> env 동기화
    if hasattr(st, "secrets"):
        try:
            for key in ("SUPABASE_URL", "SUPABASE_ANON_KEY", "SUPABASE_PUBLISHABLE_KEY", "SUPABASE_SERVICE_ROLE_KEY"):
...
def hash_password(login_id: str, password: str) -> str:
    raw = f"{AUTH_SALT}:{normalize_login_id(login_id)}:{password}"
    return hashlib.sha256(raw.encode("utf-8")).hexdigest()


def init_local_auth_db() -> None:
...
def is_valid_supabase_key(candidate: Optional[str]) -> bool:
    if not candidate:
        return False
    ...
    if value.startswith("sb_publishable_"):
        return len(value) > 20
    # Legacy JWT-style anon/service key should have 3 segments
...
def get_supabase_key() -> Optional[str]:
    for key_name in ("SUPABASE_SERVICE_ROLE_KEY", "SUPABASE_ANON_KEY", "SUPABASE_PUBLISHABLE_KEY", "SUPABASE_KEY"):
        value = get_api_key(key_name)
        if is_valid_supabase_key(value):
            return value
    return None
...
def get_chat_model(model_name: str, streaming: bool = False) -> Any:
    if model_name == "gpt-5.5":
        ...
        if not api_key:
            raise RuntimeError("OPENAI_API_KEY가 필요합니다.")
        return ChatOpenAI(model="gpt-5.5", temperature=1, streaming=streaming, openai_api_key=api_key)
...
def ensure_api_keys_from_sidebar() -> None:
    if st.session_state.openai_input_key:
        os.environ["OPENAI_API_KEY"] = st.session_state.openai_input_key.strip()
    if st.session_state.anthropic_input_key:
        os.environ["ANTHROPIC_API_KEY"] = st.session_state.anthropic_input_key.strip()
    if st.session_state.gemini_input_key:
        os.environ["GOOGLE_API_KEY"] = st.session_state.gemini_input_key.strip()


def require_supabase() -> Client:
    if supabase is None:
        raise RuntimeError("SUPABASE_URL/SUPABASE_ANON_KEY가 필요합니다.")
    return supabase
...
def sign_up(login_id: str, password: str) -> Tuple[bool, str]:
    try:
        normalized = normalize_login_id(login_id)
        with sqlite3.connect(str(AUTH_DB_PATH)) as conn:
            existing = conn.execute(
                "select user_id from app_users_local where login_id = ?",
                (normalized,),
            ).fetchone()
            if existing:
                return False, "이미 존재하는 로그인 ID입니다."

            conn.execute(
                """
                insert into app_users_local (user_id, login_id, password_hash, created_at)
                values (?, ?, ?, ?)
                """,
                (
                    str(uuid.uuid4()),
            ...
            conn.commit()
        return True, "회원가입이 완료되었습니다. 바로 로그인해서 사용해주세요."
    except Exception as exc:
        return False, f"회원가입 실패(local_auth.db 확인): {exc}"


def sign_in(login_id: str, password: str) -> Tuple[bool, str]:
    try:
        normalized = normalize_login_id(login_id)
        with sqlite3.connect(str(AUTH_DB_PATH)) as conn:
            row = conn.execute(
                "select user_id, login_id, password_hash from app_users_local where login_id = ?",
                (normalized,),
            ).fetchone()

        if not row:
            return False, "로그인 ID 또는 비밀번호가 올바르지 않습니다."

        user_id, stored_login_id, stored_hash = row
        if stored_hash != hash_password(normalized, password):
            return False, "로그인 ID 또는 비밀번호가 올바르지 않습니다."

        st.session_state.auth_user_id = user_id
        st.session_state.auth_user_email = stored_login_id or normalized
        return True, "로그인 성공"
    except Exception as exc:
...
def retitle_current_session() -> bool:
    question, answer = first_question_answer()
    if not question or not answer:
        st.warning("제목을 보정하려면 최소 한 번의 질문/답변이 필요합니다.")
        return False
    st.session_state.current_session_title = generate_session_title(question, answer)
    return save_session(st.session_state.current_session_id)
...
def save_session(session_id: Optional[str] = None) -> bool:
    try:
        ...
        sb.table("messages").delete().eq("session_id", session_id).eq("user_id", user_id).execute()
        rows = []
...
def get_sessions() -> List[Dict[str, Any]]:
    if supabase is None or not st.session_state.auth_user_id:
        return []
    try:
        result = (
...
def load_session(session_id: str) -> bool:
    try:
        ...
        sb = require_supabase()
        session_result = (
            sb.table("sessions").select("*").eq("id", session_id).eq("user_id", user_id).single().execute()
        )
        ...
        st.session_state.chat_history = [{"role": row["role"], "content": row["content"]} for row in (messages_result.data or [])]
        return True
    except Exception as exc:
...
def delete_session(session_id: str) -> bool:
    try:
        ...
        check_result = sb.table("sessions").select("id").eq("id", session_id).eq("user_id", user_id).limit(1).execute()
        if check_result.data:
            st.error("세션 삭제가 완료되지 않았습니다. RLS 정책을 확인해주세요.")
            return False
        reset_screen()
        return True
    except Exception as exc:
...
def save_documents_to_supabase(chunks: List[Any], file_name: str, session_id: str) -> int:
    ...
    for idx, chunk in enumerate(chunks):
        ...
        metadata.update({"file_name": file_name, "chunk_index": idx, "user_id": user_id, "session_id": session_id})
        prepared.append(
            {
                "content": content,
    ...
    if links:
        sb.table("session_documents").upsert(links, on_conflict="session_id,document_chunk_id").execute()
    return len(links)
...
def process_uploaded_files(uploaded_files: List[Any]) -> None:
    ...
    for index, uploaded_file in enumerate(uploaded_files):
        ...
        try:
            ...
            saved_count = save_documents_to_supabase(chunks, uploaded_file.name, st.session_state.current_session_id)
    ...
    try:
        ...
        result = supabase.rpc(
            "match_session_documents",
            {
                ...
                "match_user_id": st.session_state.auth_user_id,
...
def vector_file_names(session_id: Optional[str] = None) -> List[str]:
    ...
    try:
        query = (
            ...
            .select("file_name,session_id,user_id")
            .eq("user_id", st.session_state.auth_user_id)
        )
        if session_id:
...
def render_signup_page() -> None:
    st.title("회원가입")
    st.caption("Supabase Auth 없이 로그인 ID/비밀번호로 회원가입합니다.")
    email = st.text_input("로그인 ID", key="signup_email")
    password = st.text_input("비밀번호", type="password", key="signup_password")

    col1, col2 = st.columns(2)
    with col1:
        if st.button("회원가입 요청", use_container_width=True):
            if not email or not password:
                ...
                if ok:
                    ...
                    st.error(message)
    with col2:
        if st.button("로그인 페이지로", use_container_width=True):
            st.session_state.current_page = "main"
            st.rerun()


def render_sidebar() -> None:
    st.sidebar.title("🌈 멀티유저 세션 관리")
    st.sidebar.text_input("OpenAI API Key", type="password", key="openai_input_key")
    st.sidebar.text_input("Anthropic API Key", type="password", key="anthropic_input_key")
    st.sidebar.text_input("Gemini API Key", type="password", key="gemini_input_key")
    ensure_api_keys_from_sidebar()

    st.sidebar.divider()
    st.sidebar.subheader("커스텀 로그인(ID/PW)")
    login_email = st.sidebar.text_input("로그인 ID", key="login_email")
    login_password = st.sidebar.text_input("로그인 비밀번호", type="password", key="login_password")

    col_auth_1, col_auth_2 = st.sidebar.columns(2)
    with col_auth_1:
    ...
    if st.session_state.auth_user_email:
        st.sidebar.success(f"로그인 사용자: {st.session_state.auth_user_email}")
    else:
        st.sidebar.warning("로그인 후 세션 저장 기능을 사용할 수 있습니다.")
    ...
    logged_in = bool(st.session_state.auth_user_id)
    sessions = get_sessions() if logged_in else []
    ...
    if logged_in and selected_session_id and st.session_state.loaded_session_selector != selected_session_id:
    ...
    with c1:
        if st.button("세션저장", use_container_width=True, disabled=not logged_in):
    ...
    with c2:
        if st.button(
            ...
            disabled=(not logged_in) or (selected_session_id is None),
        ):
    ...
    with c3:
        if st.button(
            "세션삭제",
            use_container_width=True,
            disabled=(not logged_in) or (delete_target_id is None),
        ):
            if delete_target_id and delete_session(delete_target_id):
                st.sidebar.success("삭제 완료")
                st.rerun()
    with c4:
        if st.button("화면초기화", use_container_width=True, disabled=not logged_in):
            reset_screen()
            st.rerun()

    if st.sidebar.button("제목보정", use_container_width=True, disabled=not logged_in):
        if retitle_current_session():
            st.sidebar.success("제목 보정 완료")
            st.rerun()

    if st.sidebar.button("vectordb", use_container_width=True, disabled=not logged_in):
        st.session_state.show_vectordb = not st.session_state.show_vectordb
    if logged_in and st.session_state.show_vectordb:
    ...
    st.sidebar.divider()
    uploaded_files = st.sidebar.file_uploader("PDF 파일", type=["pdf"], accept_multiple_files=True)
    if st.sidebar.button("파일 처리하기", use_container_width=True, disabled=not logged_in):
        try:
